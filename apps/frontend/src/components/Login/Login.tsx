 ...
 const [values, setValues] = useState<LoginFormValues>({ username: '', password: '', confirmPassword: '' })
  ...
  function validate(next: LoginFormValues): FieldErrors {
    ...
    if (mode === 'register') {
      ...
      else if (next.password && next.confirmPassword !== next.password) e.confirmPassword = 'Les mots de passe ne correspondent pas'
    }
    return e
  }
    ...
    <div className="min-h-screen w-full flex items-center justify-center bg-gradient-to-br from-neutral-950 via-neutral-900 to-neutral-800 p-4">
      ...
      >
        ...
        <div className="space-y-1">
          <label htmlFor="username" className="block text-xs font-medium uppercase tracking-wide text-neutral-300">Username</label>
          <input
            id="username"
            name="username"
            type="text"
          ...
          />
          {errors.username && <span className="block text-[11px] font-medium text-red-400">{errors.username}</span>}
        </div>

        <div className="space-y-1">
          <label htmlFor="password" className="block text-xs font-medium uppercase tracking-wide text-neutral-300">Mot de passe</label>
          <div
          ...
          {errors.password && <span className="block text-[11px] font-medium text-red-400">{errors.password}</span>}
        </div>
        ...
        <div className="pt-1 text-center space-y-1">
          {mode === 'login' ? (
            <p className="text-[11px] text-neutral-500">Pas de compte ?{' '}
              <button
                type="button"
            ...
            <p className="text-[11px] text-neutral-500">Déjà un compte ?{' '}
              <button
                type="button"
                onClick={() => { setMode('login'); setErrors({}); setValues(v => ({ ...v, password: '', confirmPassword: '' })); }}
                className="text-indigo-400 hover:text-indigo-300 font-medium"
              >Se connecter</button>
            </p>
          )}
